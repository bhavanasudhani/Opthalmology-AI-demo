...
DiseaseRetrievalAgent = rag_agent.DiseaseRetrievalAgent
...
def load_agents():
    ...
    return {
        ...
        "relevance": RelevanceChecker()  # New Agent
    }
...
st.sidebar.info(
    """
    **Multi-Agent Design:**
    - 🛡️ **Relevance Gate**: Filters non-medical input
...
if st.button("Analyze Symptoms"):
    if not symptom_text:
        ...
        # --- RELEVANCE GATE ---
        is_relevant, reason = agents["relevance"].check_relevance(symptom_text)
        
        if not is_relevant:
            # FAILURE BEHAVIOR
            st.warning("⚠️ **Input Rejected**")
            st.error(
                f"""
                **Input not recognized as eye-related symptoms.**
                
                *Reason: {reason}*

                Please describe symptoms such as blurred vision, eye pain, redness, or light sensitivity.
                
                **No medical inference was performed.**
                """
            )
            # Display 0% Confidence
